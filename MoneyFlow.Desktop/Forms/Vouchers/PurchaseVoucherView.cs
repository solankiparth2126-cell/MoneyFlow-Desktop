...
/// <summary>
/// Executive Ledger Desktop — Purchase Voucher View (F9).
/// Embedded view hosted inside the unified VoucherShell.
/// </summary>
public class PurchaseVoucherView : UserControl, IVoucherView
{
    private readonly IVoucherHost _host;
    private readonly IAccountingService _accountingService;

    private VoucherType? _purchaseVoucherType;
    ...
    private enum ActivePurchaseSelector { None, Party, PurchaseLedger }
    private ActivePurchaseSelector _activeSelector = ActivePurchaseSelector.None;

    public VoucherTypeEnum VoucherType => VoucherTypeEnum.Purchase;
    public string VoucherTitle => "Purchase Voucher (F9)";
    public string ActiveVoucherKey => "Purchase";
    public Control ContentControl => this;

    public PurchaseVoucherView(
        IVoucherHost host,
    ...
    private void InitializeComponent()
    {
        ...
        // --- Row 1: Supplier A/c (Cr) & Purchase Ledger (Dr) ---
        ...
        _cmbParty.Enter += (s, e) =>
        {
            _activeSelector = ActivePurchaseSelector.Party;
            _cmbParty.BackColor = ExecLedgerTheme.PrimarySelection;
            _host.FlyoutPanel.Visible = true;
            _host.FlyoutPanel.SetTitle("List of Supplier / Creditor Accounts");
        ...
        _cmbPurchaseLedger = new ComboBox
        {
            DropDownStyle = ComboBoxStyle.DropDownList,
            Width = 200,
            Font = ExecLedgerTheme.UIRegular9,
        ...
        _cmbPurchaseLedger.Enter += (s, e) =>
        {
            _activeSelector = ActivePurchaseSelector.PurchaseLedger;
            _cmbPurchaseLedger.BackColor = ExecLedgerTheme.PrimarySelection;
            _host.FlyoutPanel.Visible = true;
            _host.FlyoutPanel.SetTitle("List of Purchase Accounts");
            _host.FlyoutPanel.LoadLedgers(_purchaseLedgers, includeEndOfList: false);
        };
        _cmbPurchaseLedger.Leave += (s, e) =>
        {
            _cmbPurchaseLedger.BackColor = Color.White;
        };

        _lblPurchaseBalance = new Label
        {
            Text = "Cur Bal: ₹0.00",
            Font = ExecLedgerTheme.UIBold9,
            ForeColor = ExecLedgerTheme.PrimaryNavy,
        ...
        pnlAccountRow.Controls.Add(lblPurchaseTag);
        pnlAccountRow.Controls.Add(_cmbPurchaseLedger);
        pnlAccountRow.Controls.Add(_lblPurchaseBalance);
        ...
        {
            HeaderText = "Item / Expense Description",
            Name = "ColDesc",
            Width = 320
        };
        ...
        _btnPrint = _host.CreateActionButton("Ctrl+P: Print", false, () => MessageBox.Show("Purchase voucher print preview ready.", "Executive Ledger Print", MessageBoxButtons.OK, MessageBoxIcon.Information));
    ...
    {
        if (_activeSelector == ActivePurchaseSelector.Party)
        {
            if (ledger != null)
                _cmbParty.SelectedValue = ledger.LedgerId;
            _cmbPurchaseLedger.Focus();
        }
        else if (_activeSelector == ActivePurchaseSelector.PurchaseLedger)
        {
            if (ledger != null)
                _cmbPurchaseLedger.SelectedValue = ledger.LedgerId;
            _dgvItems.Focus();
        }
    }
    ...
    public async Task InitializeAsync()
    {
        ...
        try
        {
            ...
            _purchaseVoucherType = await _accountingService.GetVoucherTypeByEnumAsync(VoucherTypeEnum.Purchase);

            await RefreshVoucherNumberPreviewAsync();

            _partyLedgers = await _accountingService.GetSupplierPartyLedgersAsync(company.CompanyId);
        ...
        {
            MessageBox.Show($"Failed to initialize Purchase Voucher: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
        finally
        {
    ...
    {
        if (_purchaseVoucherType == null || _host.CompanyContext.CurrentCompany == null || _host.CompanyContext.CurrentFinancialYear == null)
            return;

        var nextNumber = await _accountingService.GetNextVoucherNumberPreviewAsync(
            _host.CompanyContext.CurrentCompany.CompanyId,
            _purchaseVoucherType.VoucherTypeId,
    ...
    {
        if (_isInitializing) return;
        if (_cmbPurchaseLedger.SelectedItem is LedgerSummaryDto selected && _host.CompanyContext.CurrentCompany != null)
        {
            var balance = await _accountingService.GetLedgerBalanceAsync(_host.CompanyContext.CurrentCompany.CompanyId, selected.LedgerId);
    ...
    private void RecalculateTotals()
    {
        ...
        {
            _lblBalanceStatus.Text = netTotal <= 0 ? "Enter line items" : "Select Supplier and Purchase accounts";
    ...
    private async Task SaveVoucherAsync(bool closeAfterSave)
    {
        try
        {
            ...
            if (_purchaseVoucherType == null)
            {
                MessageBox.Show("Purchase voucher type not found.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            if (_cmbParty.SelectedValue is not int partyLedgerId || partyLedgerId <= 0)
            {
                MessageBox.Show("Please select a Supplier / Party account to credit.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                _cmbParty.Focus();
                return;
            }
            ...
            {
                MessageBox.Show("Please select a Purchase account to debit.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                _cmbPurchaseLedger.Focus();
                return;
            }
            ...
            var entries = new List<VoucherEntryDto>
            {
                ...
                new()
                {
                    ...
                    Debit = 0,
                    Credit = netTotal,
                    Narration = $"Purchase from {((LedgerSummaryDto)_cmbParty.SelectedItem!).LedgerName}"
                }
            };
            ...
            var voucherDto = new VoucherCreateDto
            {
                ...
                VoucherTypeId = _purchaseVoucherType.VoucherTypeId,
            ...
            MessageBox.Show(
                $"Purchase Voucher '{savedVoucher.VoucherNumber}' of ₹{netTotal:N2} saved successfully.",
                "Purchase Voucher Saved",
